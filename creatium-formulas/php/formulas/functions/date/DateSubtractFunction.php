    ...
    public function evaluate($scope)
    {
        ...
        if (Typing::isNull($unit)) {
            ...
            if (!in_array($unit, ['year', 'quarter', 'week', 'month', 'day', 'hour', 'minute', 'second', 'millisecond'])) {
                throw new Exception('dateSubtract3');
            }
        } else {
            throw new Exception('dateSubtract1 :: ' . Typing::getType($unit));
        }
        ...
        if (Typing::isNull($input)) {
            ...
            if (Typing::isNull($amount)) {
                ...
                if (intval($amount) == $amount) {
                    return $this->subtractTime($input, $unit, $amount);
                } else {
                    throw new Exception('dateSubtract4');
                }
            } else {
                throw new Exception('dateSubtract4');
            }
        } else {
            throw new Exception('dateSubtract2');
        }
    }

    protected function subtractTime($date, $unit, $amount)
    {
        $newDate = clone $date;

...
class AtanhFunction extends AbstractFunction
{
    ...
    public function evaluate($scope)
    {
        ...
        if (!Typing::isNumber($number)) {
            throw new Exception("fn7 :: atanh," . Typing::getType($number));
        }

        if ($number < -1 || $number > 1) {
            throw new Exception("atanh2 :: " . Convertation::toString($number));
        }
    ...
    {
        return [
            '$atanh' => $this->arguments[0]->toMongoExpression($localVariables, $fieldNames, $options),
        ];
    }
}
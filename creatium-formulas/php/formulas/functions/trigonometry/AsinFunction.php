...
class AsinFunction extends AbstractFunction
{
    ...
    public function evaluate($scope)
    {
        ...
        if (!Typing::isNumber($number)) {
            throw new Exception("fn7 :: asin," . Typing::getType($number));
        }

        if ($number < -1 || $number > 1) {
            throw new Exception("asin2 :: " . Convertation::toString($number));
        }
    ...
    {
        return [
            '$asin' => $this->arguments[0]->toMongoExpression($localVariables, $fieldNames, $options),
        ];
    }
}
...
class AcoshFunction extends AbstractFunction
{
    ...
    public function evaluate($scope)
    {
        ...
        if (!Typing::isNumber($number)) {
            throw new Exception("fn7 :: acosh," . Typing::getType($number));
        }

        if ($number < 1) {
            throw new Exception("acosh2 :: " . Convertation::toString($number));
        }
    ...
    {
        return [
            '$acosh' => $this->arguments[0]->toMongoExpression($localVariables, $fieldNames, $options),
        ];
    }
}